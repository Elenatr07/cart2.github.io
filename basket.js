...
const backetCountEl = document.querySelector('.cartIconWrap span');
...
function addToCart(id, name, price) {
    ...
    basket[id].count++;
    backetCountEl.textContent = getTotalBasketCount();
    basketTotalValueEl.textContent = getTotalBasketPrice().toFixed(2);
    displayProductInBasket(id);
}
...
function displayProductInBasket(id) {
    const basketRowEl = basketEl
        .querySelector(`.basketRow[data-productId="${id}"]`);
    if (!basketRowEl) {
        displayNewProductInBasket(id);
        return;
    }
...
function displayNewProductInBasket(productId) {
    const productRow = `
